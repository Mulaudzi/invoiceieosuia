...
final class IeosuiaAuthController {
    ...
    public function callback(): void {
        $this->session(); $pending=is_array($_SESSION['ieosuia_oauth']??null)?$_SESSION['ieosuia_oauth']:$this->readFlowCookie(); unset($_SESSION['ieosuia_oauth']); $this->clearFlowCookie();
        ...
        else{$db->prepare("UPDATE users SET identity_uuid=?,status='active',email_verified_at=COALESCE(email_verified_at,NOW()),updated_at=NOW() WHERE id=?")->execute([(string)$profile['sub'],$user['id']]);$user['identity_uuid']=(string)$profile['sub'];$user['status']='active';}
